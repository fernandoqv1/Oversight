...
(function () {
      ...
      <div class="cdetail-cols">
        ...
        <section class="panel">
          ...
          ${mats.length === 0 ? '<div class="empty-row" style="padding:18px;">No materials linked.</div>'
            : `<ul class="mat-list">${mats.map(m => { const siteMat = (p.materials || []).find(sm => sm.id === m.materialId || (sm.name || '').toLowerCase() === (m.name || m.materialName || '').toLowerCase()); const hzLabel = siteMat ? (typeof window.hazardTypeLabel === 'function' ? window.hazardTypeLabel(siteMat) : 'Asb') : 'Asb'; return `<li class="mat-row"><div class="mat-name">${esc(m.name || m.materialName || '—')} <span class="mat-type-tag">${esc(hzLabel)}</span></div><div class="muted small">${esc(m.location || '')}</div></li>`; }).join('')}</ul>`}
        </section>
        <section class="panel">
  ...
  function renderSamplesMaterialsPanel(allMats) {
    ...
    const matHazardTag = (m) => {
      const label = typeof window.hazardTypeLabel === 'function' ? window.hazardTypeLabel(m) : ((m.hazardType || '').toLowerCase() === 'lead' ? 'Pb' : 'Asb');
