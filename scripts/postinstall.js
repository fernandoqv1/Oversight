...
if (fs.existsSync(dt)) fs.copyFileSync(dt, path.join(libDir, 'docxtemplater.js'));
...
if (fs.existsSync(jszip)) fs.copyFileSync(jszip, path.join(libDir, 'jszip.min.js'));
...
if (fs.existsSync(fileSaverMin)) {
  fs.copyFileSync(fileSaverMin, path.join(libDir, 'file-saver.min.js'));
} else if (fs.existsSync(fileSaver)) {
  fs.copyFileSync(fileSaver, path.join(libDir, 'file-saver.min.js'));
}

// xlsx
const xlsx = path.join(root, 'node_modules/xlsx/dist/xlsx.full.min.js');
if (fs.existsSync(xlsx)) fs.copyFileSync(xlsx, path.join(libDir, 'xlsx.full.min.js'));
...
if (fs.existsSync(fontSourceDir)) {
  fontWeights.forEach((w) => {
    ...
    if (fs.existsSync(src)) fs.copyFileSync(src, path.join(fontsDir, f));
  });
}
