...
async function walkPhotos(udid, remoteDir, relDir, dateFilter, photos) {
  ...
  for (const entry of entries) {
    ...
    if (IMAGE_EXTENSIONS.has(ext)) {
      ...
      if (dateFilter) {
        if (!isoDate || isoDate !== dateFilter) continue;
      }
      photos.push({
        path: relPath,
        name: entry,
...
async function importPhotos(udid, filePaths, destDir, onProgress) {
  if (!udid) {
    return { success: false, error: 'Device UDID is required' };
  }
  ...
  for (const filePath of filePaths) {
    ...
    report({ phase: 'copying', completed: done, total, name: fileName });
    try {
      await runTool('afcclient.exe', ['-u', udid, 'get', remote, localPath], 120000);
      done += 1;
      if (fs.existsSync(localPath)) {
        imported.push({ name: fileName, localPath });
        report({ phase: 'copying', completed: done, total, name: fileName, ok: true, localPath });
      } else {
        errors.push(`Timed out copying '${fileName}'`);
        report({ phase: 'copying', completed: done, total, name: fileName, ok: false });
      }
    } catch (err) {
      done += 1;
